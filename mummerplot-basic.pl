...
my %TERMSIZE =
    (
     ...
     $PNG => { $SMALL => 600, $MEDIUM => 1024, $LARGE => 1400 }  # image pix
     );
...
sub LayoutIDs ($$)
{
	...
	#print STDERR "\nANNA: $sR-$eR ($dR)  $sQ-$eQ ($dQ) ";
	#print STDERR "\nANNA: slope=$slope loR=$loR loQ=$loQ hiR=$hiR hiQ=$hiQ";
...
sub PlotData ($$$)
{
    ...
    foreach $align ( sort { $a->[4] <=> $b->[4] } @$aref ) {
        ...
        #print STDERR "\nANNA 1 ref: $sR-$eR ($refoff, $reflen, $refdir) ";
        #print STDERR "\nANNA 1 qry: $sQ-$eQ ($qryoff, $qrylen, $qrydir) ";
        ...
        #print STDERR "\nANNA 2 ref: $sR-$eR ($refoff, $reflen, $refdir) ";
        #print STDERR "\nANNA 2 qry: $sQ-$eQ ($qryoff, $qrylen, $qrydir) ";
        ...
        if ( $OPT_coverage ) {
            ...
            foreach $fh ( @fha ) {
		# the items will get sorted by the first number
		if ($sR < $eR) {
		    print $fh "$sR-$eR,$sQ-$eQ,$refdir:$qrydir\n";
		} else {
                    print $fh "$eR-$sR,$sQ-$eQ,$refdir:$qrydir\n";
		}
                ## print $fh "$sR $sQ $sim\n", "$eR $eQ $sim\n\n\n";
	    }
        }            
...
sub WriteGP ($$)
{
    ...
    foreach ( $OPT_terminal ) {
        ...
        /^$PNG/    and do {
            $P_TERM = $OPT_gpstatus == 0 ?
                "$PNG tiny size 640,480" : "$PNG small";
    ...
    if ( scalar (@refk) == 1 ) {
        $xlabel = $refk[0];
        $xrange = $rref->{$xlabel}[1];
	print GFILE "x:$xlabel:1\n";
	print GFILE "x::$xrange\n";
    }
    else {
        $xrange = 0;
        foreach $xlabel ( sort { $rref->{$a}[0] <=> $rref->{$b}[0] } @refk ) {
            #$xrange += $rref->{$xlabel}[1]; #orig
            $xrange += ($rref->{$xlabel}[1] - 1); #anna
            $tic = $rref->{$xlabel}[0] + 1;
            $dir = ($rref->{$xlabel}[2] == 1) ? "" : "*";
            print GFILE "x:$xlabel:$tic\n";
            #print GFILE "x,$dir$xlabel,$tic,\n";
	    #print STDERR "\nANNA $xlabel: $rref->{$xlabel}[1]";
        }
	$xrange += 1;
        print GFILE "x::$xrange\n";
        $xlabel = "REFERENCE";
    ...
    if ( $OPT_coverage ) {
        ...
        $ylabel = $qryk[0];
        $yrange = $qref->{$ylabel}[1];
	print GFILE "y:$ylabel:1\n";
	print GFILE "y::$yrange\n";
    }
    else {
        $yrange = 0;
        foreach $ylabel ( sort { $qref->{$a}[0] <=> $qref->{$b}[0] } @qryk ) {
            ...
            $tic = $qref->{$ylabel}[0] + 1;
            $dir = ($qref->{$ylabel}[2] == 1) ? "" : "*";
            print GFILE "y:$ylabel:$tic\n";
            #print GFILE "y,$dir$ylabel,$tic\n";
        }
        print GFILE "y::$yrange\n";
        $ylabel = "QUERY";
    }
    ...
    #-- ranges
    #if ( defined $OPT_xrange ) { print GFILE "xrange $OPT_xrange\n"; }
    #else                       { print GFILE "xrange [1:$xrange]\n"; }

    #if ( defined $OPT_yrange ) { print GFILE "yrange $OPT_yrange\n"; }
    #else                       { print GFILE "yrange [1:$yrange]\n"; }
