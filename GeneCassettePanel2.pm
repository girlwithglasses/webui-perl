...
sub colorAllocates {
    ...
    #$self->{color_off_white} = $im->colorAllocate( 250, 250, 250 );
...
sub addFusedGeneBox {
    my ( $self, $xstart, $xend, $gene_oid, $label, $type ) = @_;

    my $gene_fusion_map = $self->{gene_fusion_map};
    my $im              = $self->{im};
    ...
    #$im->setThickness(4);
    $im->dashedLine(
        $self->coord2x($xstart), $y1, $self->coord2x($xend), $y1,
        $self->{color_red}
    );

    $im->dashedLine(
        $self->coord2x($xstart), $y2, $self->coord2x($xend), $y2,
        $self->{color_red}
    );

    $im->dashedLine(
        $self->coord2x($xstart), $y1, $self->coord2x($xstart), $y2,
        $self->{color_red}
    );

    $im->dashedLine(
        $self->coord2x($xend), $y1, $self->coord2x($xend), $y2,
        $self->{color_red}
    );
...
sub makeMapString {
    ...
    for my $r (@$gene_fusion_map) {
        ...
        #          . "&page=fusionGeneList&gene_oid=$gene_oid"
        #          . "&type=$type' ";
        $s .= "href='$gene_page_base_url&gene_oid=$gene_oid' ";
