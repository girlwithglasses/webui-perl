############################################################################
# RadialTree.pm - component to create phylogenetic tree views.
#                 Creates an inline image for a phylogenetic tree view
#                 (obtained from MG-RAST - anl.gov).
#                 Used by RadialPhyloTree.pm
# $Id: RadialTree.pm 31532 2014-07-30 18:17:03Z jinghuahuang $
############################################################################
package RadialTree;
use strict;
use warnings;
...
Returns the html output of the RadialTree component.

=cut

############################################################################
sub getNewickString {
    # mode=0: nodes only, no counts
    # mode=1: both nodes and counts
    my ($self, $mode) = @_;

    my $nNodes = keys %{$self->{nodes}};
    my @domainIndeces = ();
    for my $n ( 1 .. $nNodes ) {
        push( @domainIndeces, $n ) 
            if ( $self->{nodes}->{$n}->{level} eq 0 );
    }
    my $newick = $self->getNewickString_recursive(\@domainIndeces, $mode);
    return "(" . $newick . ");";

}

############################################################################
sub getNewickString_recursive {
    # mode=0: nodes only, no counts
    # mode=1: both nodes and counts
    my ( $self, $rootInd_aref, $mode ) = @_;
    my @rootIndeces = @$rootInd_aref;
    my $s;
    my %levelNames_h = (
        "0"=>"domain",  "1"=>"phylum", 
        "2"=>"order",   "3"=>"family"
    ...
    for my $n ( 0 .. @rootIndeces-1 ) {
        my $r = $rootIndeces[$n];
        my $level = $self->{nodes}->{$r}->{level};

        next if ( $level > 3 );
        # TODO level larger than 4 is not expected,
        # level=4 occurs once in a while due to a bug in tree formation.
        # Onygenales exists under both Eukaryota::Ascomycota
        # and Eukaryota::Arthropoda, causing confusion in the structure
        # of phylo tree.  - yjlin 20130604

        my $children_aref = $self->{nodes}->{$r}->{children};
        my $s2 = $self->getNewickString_recursive( $children_aref, $mode );

        my $levelName = $levelNames_h{$level};
        my $nodeName = $self->{nodes}->{$r}->{name};
        my $counts_aref = $self->{nodes}->{$r}->{value};

        if ( $s2 eq '' ) {
            $s2 = "$levelName-$nodeName";
        } else {
            $s2 = "($s2)$levelName-$nodeName";
        }
        if ( $n ne 0 ) {
            $s .= ",";
        }
...
sub output {
    ...
    unless ( defined( $self->data() ) ) {
        return 'No data passed to RadialTree. Use RadialTreeObject->data($data) to set.';
    }
    ...
    if ( $self->show_leaf_weight ) {
        $leaf_weight_space =
          $self->{leaf_weight_space} || $self->level_distance;
    }

    # get font
    unless ( $self->ttf ) {
        return 'No path to true type font set in RadialTree. Use $RadialTreeObject->ttf($fontname) to set.';
    }
    my $courier = $self->ttf;
        ...
        for ( my $i = 0 ; $i < scalar(@node_lineage) ; $i++ ) {
            if ( defined( $level_positions->[$i] ) ) {
                if ( exists( $level_positions->[$i]->{ $node_lineage[$i] } ) ) {
                    ...
                    $level_positions->[$i]->{ $node_lineage[$i] } =
                      [ $a1, $a2 ];
                }
            }
            else {
    ...
    if ( $self->shade_titles ) {
        ...
        for ( my $ii = 0 ; $ii < scalar(@$shading_elements) ; $ii++ ) {
            ...
            my $poly = myrim(
                ...
                $inner,
                $outer,
                $level_positions->[$shading_level]->{ $elem->{name} }->[0],
                $level_positions->[$shading_level]->{ $elem->{name} }->[1]
            );
            $self->image->filledPolygon( $poly,
                $cols->[ $ii % scalar(@$cols) ] );
            ...
            push(
                ...
                "<area shape='poly' coords='"
                  . join( ",",
                    map { int( $_->[0] / 2 ) . "," . int( $_->[1] / 2 ) }
                      $poly->vertices )
                  . "' id='$item_id' onmouseover='hover(event, \"$item_id\", \""
                  . $hover_component->id()
                  . "\");'>"
            );
        }
    ...
    foreach my $leaf (@$leafs) {
        ...
        unless ( $leaf->{empty} ) {
            ...
            if ( $self->show_titles ) {
                ...
                #print "output() leaf id=" . $leaf->{id} . " leaf name=" . $leaf->{name} . "<br/>\n";
                my @bounds         = GD::Image->stringFT( $self->colors->[1],
                    $courier, $self->font_size, 0, 0, 0, $leaf_disp_name );
                while ( ( $bounds[2] - $bounds[0] ) > ( $title_space - 5 ) ) {
                    chop $leaf_disp_name;
                    @bounds = GD::Image->stringFT( $self->colors->[1],
                        $courier, $self->font_size, 0, 0, 0, $leaf_disp_name );
                }
                if ( $angle2 > 450 ) {
                ...
                map {
                    if ( $_ > 200 ) { $bright = 1; }
                } $self->image->rgb( $self->image->getPixel( $x3, $y3 ) );
                if ( $self->shade_titles && !$bright ) {
                    $str_col = $self->colors->[0];
                }
                $self->image->stringFT( $str_col, $courier, $self->font_size,
                    ( 360 - $angle2 ) * RAD,
                    $x3, $y3, $leaf_disp_name );
        
                $titledLeafIds_h{$leaf->{id}} = $leaf_disp_name;
            }
                ...
                if ( $self->coloring_method eq 'split' ) {
                    if ( $self->leaf_weight_type eq 'bar' ) {
                        ...
                        for ( my $ii = 0 ; $ii < scalar(@$lws) ; $ii++ ) {
                            ...
                            $poly->addPt( $x3, $y3 );
                            ( $x3, $y3 ) =
                              getxypos( $angle - $leaf_weight_width,
                                $outer + $cend + 15 );
                            $x3 += $radius;
                            $y3 += $radius;
                            $poly->addPt( $x3, $y3 );
                            ( $x3, $y3 ) =
                              getxypos( $angle - $leaf_weight_width + $lww,
                                $outer + $cend + 15 );
                            $x3 += $radius;
                            $y3 += $radius;
                            $poly->addPt( $x3, $y3 );
                    ...
                    my ( $x3, $y3 ) =
                      getxypos( $angle - $leaf_weight_width, $outer + 15 );
                    $x3 += $radius;
                    $y3 += $radius;
                    $poly->addPt( $x3, $y3 );
                    ( $x3, $y3 ) = getxypos(
                        $angle - $leaf_weight_width,
                        $outer + 15 + $leaf_height
                    );
                    $x3 += $radius;
                    $y3 += $radius;
                    $poly->addPt( $x3, $y3 );
            ...
            my $v       = $leaf->{value};
            if (   ( $self->coloring_method eq 'difference' )
                || ( $self->coloring_method eq 'split' ) )
            {
                my $vnew = "";
                for (
                    my $ii = 0 ;
                    $ii < scalar( @{ $self->sample_names } ) ;
                    $ii++
                  )
                {
                    $vnew .=
            ...
            my $info =
              "<b>hits:</b> " . $v . "<br><b>lineage:</b> " . $leaf->{lineage};

            my $d = [];
            if ( scalar( @{ $leaf->{children} } )
                && !$self->{nodes}->{ $leaf->{children}->[0] }->{empty} )
            {
                if (   ( $self->coloring_method eq 'difference' )
                    || ( $self->coloring_method eq 'split' ) )
                {
                    ...
                    for (
                        my $hh = 0 ;
                        $hh < scalar( @{ $self->sample_names } ) ;
                        $hh++
                      )
                    {
                        my $dtcurr = 0;
                        @$d = map { $self->{nodes}->{$_}->{value}->[$hh] }
                          @{ $leaf->{children} };
                        foreach my $dv (@$d) {
                            $dtcurr += $dv;
                        }
                    ...
                    foreach my $n ( @{ $leaf->{children} } ) {
                        $info .=
                            "<tr><td style='background-color: rgb("
                          . $pie->color_set->[ $ii + 6 ]->[0] . ","
                          . $pie->color_set->[ $ii + 6 ]->[1] . ","
                          . $pie->color_set->[ $ii + 6 ]->[2]
                          . ");'>&nbsp;</td><td>"
                          . $self->{nodes}->{$n}->{name} . "</td>";
                        for (
                            my $hh = 0 ;
                            $hh < scalar( @{ $self->sample_names } ) ;
                            $hh++
            ...
            $hover_component->add_tooltip( $item_id,
                    "<table><tr><th>"
                  . $leaf->{name}
                  . "</th></tr><tr><td>"
                  . $info
            ...
            push( @map_spots,
                  ...
                  . int( $x2 / 2 ) . ","
                  . int( $y2 / 2 ) . ","
                  . int( $self->{node_size} / 4 )
                  . "' id='$item_id' onmouseover='hover(event, \"$item_id\", \""
                  . $hover_component->id()
                  . "\");' $href$onclick>" );
            $self->image->setThickness(1);
            $self->image->filledEllipse( $x2, $y2, $self->{node_size},
                $self->{node_size}, $self->colors->[0] );
            $self->image->ellipse( $x2, $y2, $self->{node_size},
                $self->{node_size}, $self->colors->[1] );
            $self->image->setThickness($thick);
        }
        ...
        if ( $curr_parent && $curr_parent ne $last_parent ) {
            ...
            my $start = $angle;
            my $num_siblings =
              scalar( @{ $self->{nodes}->{ $leaf->{parent} }->{children} } ) -
              1;
            my $stop = 270 + ( $factor * ( $curr_pos + $num_siblings ) );
            if ($num_siblings) {
                if ( $self->color_leafs_only ) {
                    myarc( $self->image, $radius, $radius, $inner, $start,
                        $stop, $self->colors->[1] );
                }
                else {
    ...
    for ( $rim = 1 ; $rim < ( $depth + 1 ) ; $rim++ ) {
        ...
        foreach my $node ( @{$nodes} ) {
            #print "<br/>\n";
            #print "output() node id=" . $node->{id} . " node name=" . $node->{name} . " node empty=" . $node->{empty} . "<br/>\n";
            ...
            unless ( $node->{empty} ) {
                ...
                if ( $self->color_leafs_only ) {
                    $self->image->line( $p->[0], $p->[1], $p->[2], $p->[3],
                        $self->colors->[1] );
                }
                else {
                ...
                if ( scalar( @{ $node->{children} } )
                    && !$self->{nodes}->{ $node->{children}->[0] }->{empty} )
                {
                    if (   ( $self->coloring_method eq 'difference' )
                        || ( $self->coloring_method eq 'split' ) )
                    {
                    ...
                    # show titles
                    if ( $self->show_titles ) {
                        next if ( ! $titledLeafIds_h{$node->{id}} );
                        ...
                        if ( $angle > 450 ) {
                            my @bounds =
                              GD::Image->stringFT( $self->colors->[1],
                                $courier, $self->font_size, 0, 0, 0,
                                $node->{name} );
                            my $tl = $bounds[2] - $bounds[0];
                            ( $x3, $y3 ) =
                              getxypos( $angle - 0.5, $title + $tl );
                            $angle += 180;
                        }
                        $x3 += $radius;
                        $y3 += $radius;
                        $self->image->stringFT(
                            $self->colors->[1],
                            $courier, $self->font_size, ( 360 - $angle ) * RAD,
                            $x3, $y3, $node->{name}
                        );
                    }
                    ...
                    if ( $self->show_leaf_weight ) {
                        ...
                        my $outer = sqrt(
                            ( abs( $radius - $p2 ) * abs( $radius - $p2 ) ) +
                              ( abs( $radius - $p3 ) * abs( $radius - $p3 ) ) );
                        my $angle = $p->[4];
                        if ( $self->coloring_method eq 'split' ) {
                            if ( $self->leaf_weight_type eq 'bar' ) {
                                my $lws = $self->get_leaf_height($node);
                                my $lww =
                                  $leaf_weight_width * 2 / scalar(@$lws) - 0.1;
                                for ( my $ii = 0 ; $ii < scalar(@$lws) ; $ii++ )
                                {
                                    my $poly = new GD::Polygon;
                                    my ( $x3, $y3 ) = getxypos(
                ...
                push( @map_spots,
                      ...
                      . "\");' $href$onclick>" );
                $self->image->setThickness(1);
                $self->image->filledEllipse( $p->[2], $p->[3],
                    $self->{node_size}, $self->{node_size},
                    $self->colors->[0] );
                $self->image->ellipse( $p->[2], $p->[3], $self->{node_size},
                    $self->{node_size}, $self->colors->[1] );
                $self->image->setThickness($thick);
            }
            ...
            if ( $rim < ( $depth - 1 ) ) {
                ...
                #print "output() last_parent=$last_parent, curr_parent=$curr_parent<br/>\n";
                if ( $curr_parent ne $last_parent ) {
                    my $siblings     = $self->siblings( $node->{id} );
                    #my $num_siblings = scalar(@$siblings);
                    #print "output() num_siblings=" . $num_siblings . "<br/>\n";
                    #foreach my $sibling ( @$siblings ) {
                    #    print "output() sibling id=" . $sibling->{id} . " sibling name=" . $sibling->{name} . " <br/>\n";
                    #}
                    
                    my $start        = $p->[4];
                    my $stop = getValidStop( $parent_pos, $siblings );
                    ...
                    if ( $self->color_leafs_only ) {
                        #print "output() radius=$radius, inner=$inner, start=$start, stop=$stop<br/>\n";
                        myarc( $self->image, $radius, $radius, $inner, $start,
                            $stop, $self->colors->[1] );
                    }
                    else {
    ...
    $i2->copyResampled(
        $self->image, 0, 0, 0, 0, $self->size(), $self->size(),
        $self->size() * 2,
        $self->size() * 2
    );
    ...
    # create inline png
    my $map =
        "<map name='imap_circtree_"
      . $self->id . "'>"
      . join( "\n", @map_spots )
      . "</map>";
    my $image =
        qq~<img style="border: none; z-index: 1;" src="~
      . $i2->main_image_src()
      . qq~" usemap="#imap_circtree_~
      . $self->id . qq~">~
      . $map
      . $hover_component->output() . "<br>"
      . $div;

    if ( $self->enable_click ) {
        $image =
            "<table valign=top style='width: 1500px;'><tr><td valign=top>"
          . $image
          . "</td><td valign=top align=left><div id='phylo_detail_"
          . $self->id
          . "'></div></td></tr></table>";
    }
...
sub getValidStop {
    my ( $parent_pos, $siblings ) = @_;
    
    my $stop;
    my $index = scalar(@$siblings);
    while ( !$stop && $index > 0 ) {
        $index -= 1;
        my $last_sibling = $siblings->[ $index ];
        $stop = $parent_pos->{ $last_sibling->{id} }->[4];
        if ( $stop ) {
            last;
        }
        #else {
        #    print "<br/>\n";
        #    print "getValidStop() no stop from last sibling id=" . $last_sibling->{id} . " last sibling name=" . $last_sibling->{name} . "<br/>\n";
        #}
    }
    return $stop;    
}
...
sub shading_colors {
    ...
    unless ( defined( $self->{shading_colors} ) ) {
        my $shades   = [];
        my $triplets = [
            @{ WebColors::get_palette('circle') },
            @{ WebColors::get_palette('excel') }
        ];
...
sub nodes_for_level {
    ...
    my $nodes;
    @$nodes =
      sort { $a->{lineage} cmp $b->{lineage} } map { $self->{nodes}->{$_} }
      grep { $self->{nodes}->{$_}->{level} == $level }
      keys( %{ $self->{nodes} } );
...
sub reroot {
    ...
    foreach my $sn ( @{ $root_node->{children} } ) {
        ...
        $new_nodes =
          $self->build_reroot( $new_nodes, $self->{nodes}->{$sn}, $level );
    }
...
sub myarc {
    ...
    for ( my $a = $a1 ; $a <= $a2 ; $a += $astep ) {
        ...
        #if ( ($a1 + $astep <= $a2) && ($a <= $a1 + 100 * $astep || $a + 100 * $astep >= $a2) ) {
        #    if ( $a == $a1 ) {
        #        print "myarc() omitted ends a1=$a1, a2=$a2, a=$a, x=$x, y=$y, color=$color<br/>\n";
        #    }
        #}
        #else {
        #    $im->setPixel( $x, $y, $color );            
...
sub getxypos {
    return ( $_[1] * cos( deg2rad( $_[0] ) ), $_[1] * sin( deg2rad( $_[0] ) ) );
}
...
sub coloring_method {
    ...
    if ( defined($method) ) {
        ...
        if ( $method eq 'difference' or $method eq 'split' ) {
            $self->{total_value} = [];
            for ( my $ii = 0 ;
                $ii < scalar( @{ $self->sample_names } ) ; $ii++ )
            {
...
sub get_color {
    ...
    if ( $self->coloring_method eq "domain" ) {
        ...
        my $x =
          ( $node->{value} > 1 )
          ? int( 48 / log( $self->{total_value} ) * log( $node->{value} ) )
          : $node->{value};
        $color = $color_scale->{$x};
    }
        ...
        my $factor =
          $self->{total_value}->[$aval] / $self->{total_value}->[$bval];
        ...
        my $x =
          ( $node->{value}->[0] > 1 )
          ? int(
            48 / log( $self->{total_value}->[0] ) * log( $node->{value}->[0] ) )
          : $node->{value}->[0];
        my $color_a = $color_scale->{$x};
        my $color_b = $color_scale->{$x};
        if ( $node->{value}->[1] ) {
            $x =
              ( $node->{value}->[1] > 1 )
              ? int( 48 /
                  log( $self->{total_value}->[1] ) *
                  log( $node->{value}->[1] ) )
              : $node->{value}->[1];
            $color_b = $color_scale->{ $x + 51 };
        }
...
sub get_leaf_height {
    ...
    if ( $self->coloring_method eq 'abundance' ) {
        $height =
          ( $node->{value} > 1 )
          ? int( $height / log( $self->{total_value} ) * log( $node->{value} ) )
          : 1;
    }
    elsif ( $self->coloring_method eq 'split' ) {
        my $heights = [];
        ...
        foreach my $l (@$d) {
            ...
            if ( exists( $self->{name_level_map}->{ $level . "_" . $name } ) ) {
                ...
                if ($parent) {
                    ...
                    while ( $self->{nodes}->{$curr}->{parent} ) {
                        $root =
                          $self->{nodes}->{ $self->{nodes}->{$curr}->{parent} }
                          ->{name};
                        $lineage = $root . "; " . $lineage;
                        $curr    = $self->{nodes}->{$curr}->{parent};
                    }
...
sub save {
    ...
    my $stateFile = "$cgi_tmp_dir/$id.$sessionId.radtree";
    store( $self->image, $stateFile );
